...
func hasPathSum( root *TreeNode ,  sum int ) bool {
    ...
    hasPath:=false
    walkSum(root.Val,sum,&hasPath,root)
    return hasPath

}

func walkSum(preSum int, tgtSum int, hasPath *bool,root *TreeNode){
    if *hasPath==true{
        return
    }
    ...
    if root.Left !=nil{
        ...
        walkSum( preSum+root.Left.Val,tgtSum,hasPath,root.Left)
    }
    if root.Right !=nil{

        walkSum(preSum+root.Right.Val,tgtSum,hasPath,root.Right)
    }
}
