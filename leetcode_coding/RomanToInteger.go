...
func checkCompose(romanChar rune, tempRoman []rune) (int,bool) {
	...
	if tempRoman[len(tempRoman)-1] == 'I'|| tempRoman[len(tempRoman)-1] == 'X'|| tempRoman[len(tempRoman)-1] == 'C'{
	//查看是否可以搭配
		if slices.Contains(composeMatch[tempRoman[len(tempRoman)-1]], romanChar) {
		//可以搭配则返回组合后的结果并清空temp
			sum:=charNumMap[romanChar]-charNumMap[tempRoman[len(tempRoman)-1]]
			tempRoman=[]rune{}
			return sum,true
		}else{
			sum:=charNumMap[tempRoman[len(tempRoman)-1]]
			tempRoman = append(tempRoman, romanChar)
